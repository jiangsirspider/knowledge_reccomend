...
def bioes_decode(decode_tokens, raw_text, id2ent):
    ...
    while index_ < len(decode_tokens):
        if decode_tokens[index_] == 0:
            token_label = id2ent[1].split('_')
        else:
            token_label = id2ent[decode_tokens[index_]].split('_')
        if token_label[0].startswith('S'):
            ...
            while index_ < len(decode_tokens):
                if decode_tokens[index_] == 0:
                    temp_token_label = id2ent[1].split('_')
                else:
                    temp_token_label = id2ent[decode_tokens[index_]].split('_')
