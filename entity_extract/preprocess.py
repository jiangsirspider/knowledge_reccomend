...
class NerProcessor:
    def __init__(self, cut_sent=True, cut_sent_len=256):
        ...
        # self.cut_sent_len = args.max_seq_len
...
def convert_bert_example(ex_idx, example: InputExample, tokenizer: BertTokenizer,
    ...
    for ent in entities:
        ...
        if ent_start == ent_end:
            label_ids[ent_start] = ent2id['S_' + ent_type]
        else:
            label_ids[ent_start] = ent2id['B_' + ent_type]
            label_ids[ent_end] = ent2id['E_' + ent_type]
            for i in range(ent_start + 1, ent_end):
                label_ids[i] = ent2id['I_' + ent_type]
    ...
    try:
        ...
        print('get errors: tokens=', tokens)
        raise Exception
